...
public class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        // string directory = @"C:\Users\zgccr\Downloads\Test";
        Console.WriteLine($"当前目录: {directory}");

        Console.WriteLine("请输入最大并发数（默认为5）：");
        string maxDegreeOfParallelismString = Console.ReadLine() ?? "";
        maxDegreeOfParallelism = !string.IsNullOrWhiteSpace(maxDegreeOfParallelismString)
            ? int.Parse(maxDegreeOfParallelismString)
            : 5;

        // 提示按下回车键开始处理
        Console.WriteLine("按下回车键开始处理");
        Console.ReadLine();

        UpdateFiles(directory);

        Console.WriteLine($"处理完成，共处理 {totalFiles} 个文件，耗时 {DateTime.Now - startTime}");
        Console.WriteLine("按下回车键退出");
        Console.ReadLine();
    }

    private static void UpdateFiles(string directory)
    {
